  ...
  return (
    <div className="max-w-5xl mx-auto">
      ...
      <div className="text-center mb-8 sm:mb-12">
        <h1 className="text-2xl sm:text-4xl font-bold mb-3 sm:mb-4" style={{ color: "#2D2D2D" }}>
          How can we help you?
        </h1>
        <p className="text-sm sm:text-lg mb-6 sm:mb-8" style={{ color: "#B0B3B8" }}>
        ...
        <div className="max-w-2xl mx-auto relative">
          ...
          <Input
            ...
            className="pl-11 sm:pl-14 h-11 sm:h-14 rounded-full text-sm sm:text-base shadow-lg border-0"
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 sm:gap-6 mb-8 sm:mb-12">
        {[
          ...
          return (
            <div
              key={option.title}
              className="p-4 sm:p-6 rounded-xl sm:rounded-2xl bg-white shadow-sm border border-gray-100 hover:shadow-md hover:border-gray-200 transition-all cursor-pointer"
            >
              <div className="flex items-start justify-between mb-3 sm:mb-4">
                ...
                >
                  <Icon size={20} color={option.color} variant="Bulk" className="sm:hidden" />
                  <Icon size={24} color={option.color} variant="Bulk" className="hidden sm:block" />
                </div>
                {option.available && (
                  <Badge variant="partial" className="text-[10px] sm:text-xs">
                    <div className="h-1 w-1 sm:h-1.5 sm:w-1.5 rounded-full mr-1 sm:mr-1.5" style={{ backgroundColor: "#14462a" }} />
                    Available
                  </Badge>
                )}
              </div>
              <h3 className="text-sm sm:text-base font-semibold mb-0.5 sm:mb-1" style={{ color: "#2D2D2D" }}>
                {option.title}
              </h3>
              <p className="text-xs sm:text-sm mb-3 sm:mb-4 truncate" style={{ color: "#B0B3B8" }}>
                {option.description}
              </p>
              <Button
                variant="outline"
              ...
              >
                {option.action}
                <ExportSquare size={12} color="currentColor" className="ml-1.5 sm:ml-2" />
              </Button>
            </div>
          );
        })}
      ...
      <div className="mb-8 sm:mb-12">
        <div className="flex items-center gap-3 mb-4 sm:mb-6">
          ...
          >
            <MessageQuestion size={16} color="#F59E0B" variant="Bulk" className="sm:hidden" />
            <MessageQuestion size={20} color="#F59E0B" variant="Bulk" className="hidden sm:block" />
          </div>
          <h2 className="text-lg sm:text-xl font-semibold" style={{ color: "#2D2D2D" }}>
            Popular Articles
          </h2>
        </div>
          ...
          {popularArticles.map((article, index) => (
            <div key={article.slug}>
              <button
                ...
                className="w-full flex items-center justify-between p-3 sm:p-5 hover:bg-gray-50 transition-colors text-left group"
              >
                <div className="flex items-center gap-3 sm:gap-4 flex-1 min-w-0">
                  ...
                  >
                    <Book1 size={16} color="#14462a" variant="Bulk" className="sm:hidden" />
                    <Book1 size={20} color="#14462a" variant="Bulk" className="hidden sm:block" />
                  </div>
                  <div className="flex-1 min-w-0">
                    <h3
                      className="text-sm sm:text-base font-medium mb-0.5 sm:mb-1 group-hover:text-[#14462a] transition-colors truncate"
                      style={{ color: "#2D2D2D" }}
                    >
                      {article.title}
                    </h3>
                    <div className="flex items-center gap-2 sm:gap-3 text-[10px] sm:text-xs" style={{ color: "#B0B3B8" }}>
                ...
                <ArrowRight2
                  size={16}
                  color="#B0B3B8"
                  className="group-hover:translate-x-1 group-hover:text-[#14462a] transition-all shrink-0"
                />
              </button>
              {index < popularArticles.length - 1 && (
                <div className="h-px mx-3 sm:mx-5 bg-gray-100" />
              )}
            </div>
          ))}
        </div>
      ...
      {/* Help Categories */}
      <div className="mb-8 sm:mb-12">
        <h2 className="text-lg sm:text-xl font-semibold mb-4 sm:mb-6" style={{ color: "#2D2D2D" }}>
          Browse by Category
        </h2>
            ...
            return (
              ...
              >
                <div className="flex items-center gap-3 mb-4 sm:mb-5">
                  ...
                  >
                    <Icon size={16} color={category.color} variant="Bulk" className="sm:hidden" />
                    <Icon size={20} color={category.color} variant="Bulk" className="hidden sm:block" />
                  </div>
                  <h3 className="text-base sm:text-lg font-semibold" style={{ color: "#2D2D2D" }}>
                    {category.title}
                  </h3>
                </div>
                <div className="space-y-0.5 sm:space-y-1">
                  {category.articles.map((article) => (
                    <button
                      key={article.slug}
                      onClick={() => openArticle(article.slug)}
                      className="w-full flex items-center justify-between p-2 sm:p-3 rounded-lg sm:rounded-xl hover:bg-gray-50 transition-colors text-left group"
                    >
                      <span
                        className="text-xs sm:text-sm group-hover:text-[#14462a] transition-colors truncate"
                        style={{ color: "#2D2D2D" }}
                      >
                        {article.title}
                      </span>
                      <div className="flex items-center gap-2 shrink-0 ml-2">
                        <span className="text-[10px] sm:text-xs hidden sm:inline" style={{ color: "#B0B3B8" }}>
                          {article.views}
                        </span>
                        <ArrowRight2
                          size={12}
      ...
      >
        <h3 className="text-lg sm:text-2xl font-semibold mb-2 sm:mb-3" style={{ color: "#2D2D2D" }}>
          Still need help?
        </h3>
        <p className="text-sm sm:text-base mb-4 sm:mb-6" style={{ color: "#65676B" }}>
          Our support team is here to help you with any questions
        </p>
        <div className="flex flex-col sm:flex-row items-center justify-center gap-3 sm:gap-4">
          <Button
            className="rounded-full h-10 sm:h-12 px-6 sm:px-8 shadow-lg hover:shadow-xl transition-shadow w-full sm:w-auto text-sm"
            style={{ backgroundColor: "#14462a", color: "white" }}
          >
            <Message size={16} color="white" className="mr-2" />
            Contact Support
          </Button>
          <Button
            variant="outline"
            className="rounded-full h-10 sm:h-12 px-6 sm:px-8 w-full sm:w-auto text-sm"
            style={{ borderColor: "#14462a", color: "#14462a" }}
          >
            <Book1 size={16} color="#14462a" className="mr-2" />
            View All Guides
          </Button>
        </div>
      </div>
      ...
      {selectedArticle && currentArticle && (
        ...
        >
          <div 
            className="bg-white rounded-2xl sm:rounded-3xl max-w-2xl w-full max-h-[95vh] sm:max-h-[90vh] overflow-hidden flex flex-col shadow-2xl"
            onClick={(e) => e.stopPropagation()}
          >
            {/* Modal Header - Styled like marketing article */}
            <div 
              className="p-4 sm:p-8 pb-4 sm:pb-6"
              style={{ 
            ...
            >
              <div className="flex items-center justify-between mb-3 sm:mb-5">
                <button
                  onClick={closeArticle}
                  className="flex items-center gap-1.5 sm:gap-2 text-xs sm:text-sm text-gray-500 hover:text-gray-700 transition-colors"
                >
                ...
                <button
                  onClick={closeArticle}
                  className="h-8 w-8 sm:h-9 sm:w-9 rounded-full flex items-center justify-center hover:bg-black/5 transition-colors"
                >
                  <CloseCircle size={20} color="#65676B" />
                </button>
              </div>
              
              <Badge 
                className="mb-3 sm:mb-4 text-[10px] sm:text-xs"
                style={{ 
              ...
              <h2 className="text-lg sm:text-2xl font-bold mb-2 sm:mb-3" style={{ color: "#14462a" }}>
              ...
              <div className="flex items-center gap-2 text-xs sm:text-sm text-gray-500">
                <Clock size={14} color="#9CA3AF" variant="Bulk" />
                {currentArticle.readTime}
              </div>
            </div>

            {/* Modal Content - Rich styled content */}
            <div className="flex-1 overflow-y-auto p-4 sm:p-8 text-sm sm:text-base">
            ...
            <div className="p-4 sm:p-6 border-t border-gray-100 bg-gray-50/50">
              <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
                ...
                <div className="flex items-center gap-2 w-full sm:w-auto">
                  <Button variant="outline" size="sm" className="rounded-full px-3 sm:px-4 hover:bg-[#14462a]/5 hover:border-[#14462a]/30 flex-1 sm:flex-none h-8 text-xs sm:text-sm">
                    <TickCircle size={14} color="#14462a" variant="Bulk" className="mr-1 sm:mr-1.5" />
                    Yes
                  </Button>
                  <Button variant="outline" size="sm" className="rounded-full px-3 sm:px-4 hover:bg-red-50 hover:border-red-200 flex-1 sm:flex-none h-8 text-xs sm:text-sm">
                    <CloseCircle size={14} color="#DC2626" variant="Bulk" className="mr-1 sm:mr-1.5" />
                    No
                  </Button>
                </div>
              </div>
